    ...
    wx.login({
      success: function (ress) {
        if (ress.code) {
          ...
          wx.request({
            url: 'https://www.bozhiyingxiao.com/little_program/store_love/index.php?s=/Little/Center/carss',
            data: { code: ress.code },
            method: "GET",
  ...
  getTotalPrice() {
    ...
    for (let i = 0; i < carts.length; i++) {         // 循环列表得到每个数据
      if (carts[i].selected) {                   // 判断选中才会计算价格
        total += Number(carts[i].shop_money);     // 所有价格加起来
      }
    }
    if (flag == 1) {
      total = total - points
  ...
  delName(e) {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          wx.request({
            url: 'https://www.bozhiyingxiao.com/little_program/store_love/index.php?s=/Little/Del/index',
